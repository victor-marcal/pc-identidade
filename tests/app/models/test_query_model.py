...
def test_to_query_dict_with_ge_operator():
    ...
    assert result == {"age": {"$gte": 18}, "name": "John"}
...
def test_to_query_dict_with_gt_operator():
    ...
    assert result == {"score": {"$gt": 85.5}, "category": "A"}
...
def test_to_query_dict_with_le_operator():
    ...
    assert result == {"price": {"$lte": 100.0}, "product": "laptop"}
...
def test_to_query_dict_with_lt_operator():
    ...
    assert result == {"weight": {"$lt": 50}, "item": "package"}
...
def test_to_query_dict_multiple_operators_same_field():
    ...
    assert result == {"age": {"$gte": 18, "$lte": 65}, "name": "John"}
...
def test_to_query_dict_all_operators():
    ...
    query = TestQuery(min_score__ge=10, max_score__le=100, threshold__gt=50, limit__lt=90, category="test")
    result = query.to_query_dict()
...
def test_to_query_dict_complex_field_names():
    ...
    query = TestQuery(user_name="john_doe", user_age__ge=25, full_name__le="John Doe")
    result = query.to_query_dict()

    assert result == {"user_name": "john_doe", "user_age": {"$gte": 25}, "full_name": {"$lte": "John Doe"}}
...
def test_to_query_dict_with_mixed_types():
    ...
    query = TestQuery(name="Alice", age__ge=30, salary__le=75000.50, active=True, tags=["python", "fastapi"])
    result = query.to_query_dict()
...
def test_to_query_dict_with_default_values():
    ...
    assert result == {"status": "active", "priority": {"$gte": 1}, "category": "important"}
