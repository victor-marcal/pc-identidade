...
async def test_crud_service_find():
    ...
    mock_repo.find.assert_called_once_with(filters=filters, limit=10, offset=0, sort=None)
...
async def test_crud_service_find_with_sort():
    ...
    assert result == [mock_entity]
    mock_repo.find.assert_called_once_with(filters=filters, limit=5, offset=10, sort={"name": -1})
